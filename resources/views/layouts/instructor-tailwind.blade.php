...
<body class="bg-body antialiased min-h-screen">
    <div class="flex h-screen overflow-hidden">
        ...
        <div id="sidebar" class="sidebar-transition flex flex-col w-64 bg-primary shadow-xl fixed inset-y-0 left-0 z-50 transform -translate-x-full lg:translate-x-0 lg:static lg:inset-0">
            <!-- Sidebar Header -->
            <div class="flex items-center justify-center h-16 px-4 border-b border-[#fff]/20 flex-shrink-0">
                <div class="flex items-center gap-3">
                    ...
                    <h1 class="text-white font-semibold text-lg">শিখুন প্ল্যাটফর্ম</h1>
                </div>
            </div>
            ...
            <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
                <a href="{{ route('instructor.dashboard') }}" 
                   class="flex items-center gap-3 px-4 py-3 text-secondary-100 rounded-lg anim hover:bg-card hover:text-blue {{ request()->routeIs('instructor.dashboard') ? 'bg-card text-blue' : '' }}">
                ...
                <a href="{{ route('instructor.courses') }}" 
                   class="flex items-center gap-3 px-4 py-3 text-secondary-100 rounded-lg anim hover:bg-card hover:text-blue {{ request()->routeIs('instructor.courses*') ? 'bg-card text-blue' : '' }}">
                    <i class="fas fa-book text-lg flex-shrink-0"></i>
                    <span class="font-medium">কোর্স সমূহ</span>
                </a>
                
                <a href="{{ route('instructor.courses.create') }}" 
                   class="flex items-center gap-3 px-4 py-3 text-secondary-100 rounded-lg anim hover:bg-card hover:text-blue {{ request()->routeIs('instructor.courses.create*') ? 'bg-card text-blue' : '' }}">
                    <i class="fas fa-plus-circle text-lg flex-shrink-0"></i>
                    <span class="font-medium">নতুন কোর্স</span>
                </a>
                
                <a href="{{ route('instructor.students') }}" 
                   class="flex items-center gap-3 px-4 py-3 text-secondary-100 rounded-lg anim hover:bg-card hover:text-blue {{ request()->routeIs('instructor.students*') ? 'bg-card text-blue' : '' }}">
                    <i class="fas fa-users text-lg flex-shrink-0"></i>
                    <span class="font-medium">শিক্ষার্থীগণ</span>
                </a>
                
                <a href="{{ route('instructor.earnings') }}" 
                   class="flex items-center gap-3 px-4 py-3 text-secondary-100 rounded-lg anim hover:bg-card hover:text-blue {{ request()->routeIs('instructor.earnings*') ? 'bg-card text-blue' : '' }}">
                ...
                <a href="{{ route('instructor.enrollments') }}" 
                   class="flex items-center gap-3 px-4 py-3 text-secondary-100 rounded-lg anim hover:bg-card hover:text-blue {{ request()->routeIs('instructor.enrollments*') ? 'bg-card text-blue' : '' }}">
                ...
                <a href="{{ route('instructor.certificates') }}" 
                   class="flex items-center gap-3 px-4 py-3 text-secondary-100 rounded-lg anim hover:bg-card hover:text-blue {{ request()->routeIs('instructor.certificates*') ? 'bg-card text-blue' : '' }}">
                    <i class="fas fa-certificate text-lg flex-shrink-0"></i>
                    <span class="font-medium">সার্টিফিকেট</span>
                </a>
                
                <a href="{{ route('instructor.coupons') }}" 
                   class="flex items-center gap-3 px-4 py-3 text-secondary-100 rounded-lg anim hover:bg-card hover:text-blue {{ request()->routeIs('instructor.coupons*') ? 'bg-card text-blue' : '' }}">
                    <i class="fas fa-tags text-lg flex-shrink-0"></i>
                    <span class="font-medium">কুপন কোড</span>
                </a>
                
                <a href="{{ route('instructor.profile') }}" 
                   class="flex items-center gap-3 px-4 py-3 text-secondary-100 rounded-lg anim hover:bg-card hover:text-blue {{ request()->routeIs('instructor.profile*') ? 'bg-card text-blue' : '' }}">
            ...
            <header class="bg-card border-b border-[#fff]/20 px-4 py-4 lg:px-6 flex-shrink-0">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-4">
                        <button id="sidebar-toggle" class="lg:hidden p-2 text-secondary-100 hover:text-blue anim">
                    ...
                    <div class="flex items-center gap-4">
                        <!-- Theme Toggle -->
                        <div class="relative">
                            <button id="theme-toggle" class="p-2 text-secondary-100 hover:text-blue anim relative">
