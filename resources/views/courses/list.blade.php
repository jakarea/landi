                ...
                <form method="GET" action="{{ route('courses') }}" class="filter-form">
                    <div class="grid grid-cols-1 lg:grid-cols-12 items-center">
                        <div class="w-full flex gap-x-5 items-center lg:col-span-4">
                            <label for="search" class="text-[#fff] text-sm lg:text-base font-normal">কোর্স খুঁজুন</label>
                            <input type="text" class="text-[#fff] border border-[#fff]/20 rounded-md px-3 py-2 font-normal text-sm min-w-[65%]" id="search" name="search"
                                value="{{ $search }}" placeholder="কোর্সের নাম লিখুন...">
                        </div>

                        <div class="w-full flex gap-x-5 items-center lg:col-span-3">
                            <label for="category" class="text-[#fff] text-sm lg:text-base font-normal">বিভাগ</label>
                            <select class="text-[#fff] border border-[#fff]/20 rounded-md px-3 py-2 font-normal text-sm min-w-[65%]" id="category" name="category">
                        ...
                        <div class="w-full flex gap-x-5 items-center lg:col-span-3">
                            <label for="sort" class="text-[#fff] text-sm lg:text-base font-normal">সাজান</label>
                            <select class="text-[#fff] border border-[#fff]/20 rounded-md px-3 py-2 font-normal text-sm min-w-[65%]" id="sort" name="sort">
                        ...
                        <div class="w-full lg:col-span-2">
                            <div class="flex gap-x-3 items-center">
                                <button type="submit" class="inline-flex font-golos justify-center items-center bg-submit rounded-md lg:rounded-[10px] p-1.5 font-medium text-sm text-[#fff] gap-x-3 anim
               hover:!bg-lime md:text-base px-3 lg:text-lg cursor-pointer
                hover:text-primary group lg:my-0 lg:order-1 border border-[#9F93A7]/70 lg:py-2 lg:px-4">
                                ...
                                @if ($search || $category || $sort != 'latest')
                                    <a href="{{ route('courses') }}" class="block font-normal text-sm lg:text-lg text-[#fff] anim hover:text-[#fff] underline">
                                        Clear
                                    </a>
                                @endif
                            </div>
                ...
                <div class="w-full p-5 lg:p-[30px] rounded-lg lg:rounded-[20px] bg-[#131620] anim hover:mt-10">
                    <div class="w-full">  
                            <img src="{{ $course->thumbnail ? asset($course->thumbnail) : asset('assets/images/default-course.jpg') }}" 
                                 alt="{{ $course->title }}" class="w-full rounded-[10px] h-[160px] object-cover"
                    ...
                    <div class="mt-10 lg:mt-[60px]">
                        <h5 class="font-semibold text-sm lg:text-lg leading-[140%] text-[#E2E8F0] mb-2 lg:mb-2.5">{{ $course->title }}</h5>

                        @if($course->short_description)
                                <p class="font-normal text-xs lg:text-sm leading-[140%] text-[#ABABAB] lg:max-w-[85%]">{{ $course->short_description }}</p>
                            @endif 
