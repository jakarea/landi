...
@section('title', 'লাইভ ক্লাস সম্পাদনা করুন')
@section('header-title', 'লাইভ ক্লাস')
@section('header-subtitle', 'লাইভ ক্লাস সম্পাদনা করুন')
...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-white">লাইভ ক্লাস সম্পাদনা করুন</h1>
            <p class="text-secondary-200 mt-1">{{ $liveClass->title }}</p>
        </div>
    ...
    <div class="glass-effect rounded-xl border border-white/20 p-8">
        <form action="{{ route('instructor.live-classes.update', $liveClass) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="form-group">
                ...
                <textarea name="description" 
                          ...
                          rows="4"
                          class="w-full bg-white/10 border border-white/20 rounded-lg px-4 py-3 text-white placeholder-white/60 focus:border-blue focus:outline-none transition-colors"
                          placeholder="এই ক্লাসে কি শেখানো হবে তার সংক্ষিপ্ত বিবরণ লিখুন...">{{ old('description', $liveClass->description) }}</textarea>
                @error('description')
                    <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <select name="course_id" 
                    ...
                    @foreach($courses as $course)
                        <option value="{{ $course->id }}" class="bg-slate-800 text-white" {{ old('course_id', $liveClass->course_id) == $course->id ? 'selected' : '' }}>
                            {{ $course->title }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="form-group">
                    <label for="start_time" class="block text-secondary-200 font-medium mb-2">
                        শুরুর সময় <span class="text-red-400">*</span>
                    </label>
                    <input type="datetime-local" 
                           name="start_time" 
                           id="start_time" 
                           value="{{ old('start_time', $liveClass->start_time->format('Y-m-d\TH:i')) }}"
                    ...
                    @error('start_time')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
                    <p class="text-secondary-300 text-xs mt-1">ভবিষ্যতের তারিখ ও সময় নির্বাচন করুন</p>
                </div>

                <div class="form-group">
                    <label for="duration_minutes" class="block text-secondary-200 font-medium mb-2">
                    ...
                    <select name="duration_minutes" 
                        ...
                        <option value="15" {{ old('duration_minutes', $liveClass->duration_minutes) == '15' ? 'selected' : '' }}>১৫ মিনিট</option>
                        <option value="30" {{ old('duration_minutes', $liveClass->duration_minutes) == '30' ? 'selected' : '' }}>৩০ মিনিট</option>
                        <option value="45" {{ old('duration_minutes', $liveClass->duration_minutes) == '45' ? 'selected' : '' }}>৪৫ মিনিট</option>
                        <option value="60" {{ old('duration_minutes', $liveClass->duration_minutes) == '60' ? 'selected' : '' }}>১ ঘন্টা</option>
                        <option value="90" {{ old('duration_minutes', $liveClass->duration_minutes) == '90' ? 'selected' : '' }}>১.৫ ঘন্টা</option>
                        <option value="120" {{ old('duration_minutes', $liveClass->duration_minutes) == '120' ? 'selected' : '' }}>২ ঘন্টা</option>
                        <option value="150" {{ old('duration_minutes', $liveClass->duration_minutes) == '150' ? 'selected' : '' }}>২.৫ ঘন্টা</option>
                        <option value="180" {{ old('duration_minutes', $liveClass->duration_minutes) == '180' ? 'selected' : '' }}>৩ ঘন্টা</option>
                    </select>
            ...
            <div class="space-y-6 bg-blue-900/20 border border-blue-800 rounded-xl p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="form-group">
                        ...
                        <input type="text" 
                               ...
                               id="zoom_meeting_id" 
                               value="{{ old('zoom_meeting_id', $liveClass->zoom_meeting_id) }}"
                               class="w-full bg-white/10 border border-white/20 rounded-lg px-4 py-3 text-white placeholder-white/60 focus:border-blue focus:outline-none transition-colors"
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" 
                               name="zoom_password" 
                               id="zoom_password" 
                               value="{{ old('zoom_password', $liveClass->zoom_password) }}"
                               class="w-full bg-white/10 border border-white/20 rounded-lg px-4 py-3 text-white placeholder-white/60 focus:border-blue focus:outline-none transition-colors"
                ...
                <div class="form-group">
                    ...
                    <input type="url" 
                           name="zoom_join_url" 
                           id="zoom_join_url" 
                           value="{{ old('zoom_join_url', $liveClass->zoom_join_url) }}"
                           class="w-full bg-white/10 border border-white/20 rounded-lg px-4 py-3 text-white placeholder-white/60 focus:border-blue focus:outline-none transition-colors"
                    ...
                    <div class="flex">
                        ...
                        <div>
                            ...
                            <ul class="text-sm space-y-1">
                                <li>• আপনার Zoom অ্যাকাউন্ট থেকে মিটিং তথ্য আপডেট করুন</li>
                                <li>• মিটিং ID এবং Join URL সঠিক আছে কি না যাচাই করুন</li>
                                <li>• পাসওয়ার্ড পরিবর্তন করা থাকলে তা আপডেট করুন</li>
                            </ul>
                        </div>
                    </div>
                </div>
            ...
            <!-- Current Status -->
            <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 text-blue-800 dark:text-blue-200 p-4 rounded-xl">
                <div class="flex items-center">
                    <i class="fas fa-info-circle mr-3"></i>
                    <div>
                        <p class="font-medium">বর্তমান স্ট্যাটাস: {{ $liveClass->status_bangla }}</p>
                    </div>
                </div>
            </div>

            @if($liveClass->status !== 'scheduled')
                <div class="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 text-yellow-800 dark:text-yellow-200 p-4 rounded-xl">
                    <div class="flex">
                        <i class="fas fa-exclamation-triangle mr-3 mt-0.5"></i>
                        <div>
                            <p class="font-medium mb-1">সতর্কতা</p>
                            <p class="text-sm">এই ক্লাসটি শুরু হয়ে গেছে বা শেষ হয়ে গেছে। পরিবর্তন সাবধানে করুন।</p>
                        </div>
                    </div>
                </div>
            @endif
